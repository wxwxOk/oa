...
**Goal:** 在现有 OA 审批中心基础上新增固定报销管理模块，让员工提交报销单、上传图片/PDF 发票，审核人员查看明细后手写签字通过。

**Target features:**
- 固定报销模块：独立菜单、报销申请列表/详情，复用现有登录、权限、审批任务和时间线能力。
- 报销申请提交：申请人填写事由、部门、总金额，并维护多条费用明细（日期/类别/金额/说明/发票）。
- 发票上传：支持图片和 PDF 本地存储，在报销详情中可查看。
- 审核签字：审核人员通过报销时必须手写签名，签名保存到审批记录并可在详情/PDF 中查看。
...
### Active
- [ ] v1.4 固定报销模块：独立菜单、权限、列表和详情，复用现有审批闭环。
- [ ] v1.4 报销申请提交：支持报销事由、部门、总金额和多条费用明细。
- [ ] v1.4 发票附件：支持图片/PDF 上传、本地存储和详情查看。
- [ ] v1.4 审批签字：审核人员通过报销时必须手写签名并留痕。
...
v1.4 采用固定报销管理模块承载报销单、费用明细、发票附件和审批签字；复用现有 OA 审批中心的登录、权限、审批任务、状态流转和时间线能力，避免把通用表单系统一次性扩展成完整文件上传平台。
...
---
*Last updated: 2026-05-02 after v1.4 milestone start; v2.0 history preserved from 2026-04-25/26*
