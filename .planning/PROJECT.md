...
## What This Is
轻量开源 OA 平台，面向中小企业。v1.2 已交付完整的组织架构管理 + 自定义表单收集系统，含 12 列栅格布局设计器、分组标题、动态行表格、PDF 保真输出、PC/Mobile 响应式填写页和 Docker 一键部署。v2.0 已扩展为表单驱动的 OA 审批中心：员工通过动态表单提交内部申请，系统按模板绑定流程流转到审批人，并把申请、审批、备注、编辑、归档导出和统计串成闭环。v1.3 已交付固定的到访信息管理模块，用于沉淀渠道往来测试表中的学员到访、咨询接待、试听跟进和成交状态管理。v1.4 当前聚焦固定报销管理模块，让员工提交报销申请、上传发票并由审核人员签字通过。
...
◆ v1.4 报销管理 started (2026-05-02)

## Current Milestone: v1.4 报销管理

**Goal:** 在现有组织架构和权限体系上新增固定报销管理模块，让员工提交报销申请、上传图片/PDF 发票，并由部门审核人员初审、财务审核人员复核后手写签字通过；本期支持明细导出。

**Target features:**
- 固定报销模块：独立菜单、报销申请列表/详情，不把自定义表单 JSONB 作为主数据模型。
- 报销申请提交：申请人填写标题、类别、发生日期、金额、事由、收款信息和备注。
- 发票上传：支持多张图片/PDF 本地附件，详情页可预览图片并下载原文件。
- 两级审核签字：部门初审 + 财务复核，通过时必须 Canvas 手写签名，驳回需记录节点和原因。
- 明细导出：授权人员可按当前筛选条件导出报销 Excel 明细，本期不做统计看板。
...
### Active
- [ ] v1.4 固定报销模块：独立菜单、权限、列表和详情，不复用自定义表单 JSONB 主模型。
- [ ] v1.4 报销申请提交：支持标题、类别、发生日期、金额、事由、收款信息和备注。
- [ ] v1.4 发票附件：支持多张图片/PDF 上传、本地存储、图片预览和原文件下载。
- [ ] v1.4 两级审核签字：部门初审 + 财务复核，通过时必须手写签名并留痕。
- [ ] v1.4 明细导出：授权人员可按筛选条件导出报销 Excel 明细。
...
v1.4 采用固定报销管理模块承载报销申请、发票附件、部门初审、财务复核、手写签字和明细导出；复用现有登录、组织架构、RBAC、状态流转和时间线经验，避免把通用表单系统一次性扩展成完整文件上传平台或财务付款系统。
...
---
*Last updated: 2026-05-02 after v1.4 requirements and roadmap definition; v2.0 history preserved from 2026-04-25/26*
