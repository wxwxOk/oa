...
gaps:
  - truth: "Disabling or deleting a process bound by published APPROVAL_REQUIRED templates is blocked"
    status: partial
    reason: "DELETE and PATCH status paths enforce the published-template binding guard, but the full process edit path can still set isActive: false without calling that guard."
    artifacts:
      - path: "backend/src/modules/approval/process.route.ts"
        issue: "updateApprovalProcessConfig writes input.isActive directly at lines 199-205 and does not call assertNotBoundByPublishedApprovalTemplate before disabling."
      - path: "frontend/src/pages/ApprovalProcessPage.vue"
        issue: "The edit dialog exposes the 启用 toggle and saves through store.update/buildPayload, so the bypass is reachable from the UI."
    missing:
      - "In updateApprovalProcessConfig, when existing.isActive is true and input.isActive is false, call assertNotBoundByPublishedApprovalTemplate(processId) before updating."
      - "Add a regression test proving full edit cannot disable a process bound by a published APPROVAL_REQUIRED template."
---
...
| 6 | Disabling or deleting a process bound by published `APPROVAL_REQUIRED` templates is blocked | FAILED | `PATCH /status` and `DELETE` call `assertNotBoundByPublishedApprovalTemplate`, but full `PUT /approval/processes/:id` writes `isActive` through `updateApprovalProcessConfig` without the guard. |
...
| 12 | Wave 0 tests, regression tests, schema validation, backend build, frontend build all support Phase 16 behavior | VERIFIED | Orchestrator evidence: backend Phase 16 approval/template/RBAC tests 39 pass, Phase 15 regression tests 16 pass after `prisma migrate deploy`, backend build passed, frontend `bun run build` passed, schema drift false. |
...
| `backend/src/modules/approval/process.route.ts` | Approval process configuration REST API | PARTIAL | CRUD/status/delete/validate routes exist and are wired; full edit disable guard gap blocks one must-have. |
...
| `approvalProcess full edit` | published-template binding guard | `isActive: false` protection | FAILED | Full edit route accepts `isActive` and does not call `assertNotBoundByPublishedApprovalTemplate`. |
...
| `process.route.ts` bound-process disable guard | `input.isActive` from full edit payload | `ApprovalProcessPage.buildPayload()` -> `store.update()` -> `PUT /approval/processes/:id` | No guard | FAILED |
...
| Full edit cannot disable a bound published process | Static path trace | Missing guard in `updateApprovalProcessConfig` | FAIL |
...
|---|---:|---|---|---|
| `backend/src/modules/approval/process.route.ts` | 199 | Full update writes `isActive` without the published-template binding guard | Blocker | Fails the Plan 16-03 must-have that disabling a process bound by published approval templates is blocked. |
...
Phase 16 mostly achieves the goal: admins can configure approval processes, bind templates, keep collection-only templates, maintain department approvers, and rely on RBAC plus required-field/schema-version safeguards.

The blocking gap is narrow but material: a published approval-required template can still be left bound to an inactive process through the full process edit route. Later roadmap phases do not explicitly address this guard, so it is not deferred.

The advisory code review also found security and UX/data-integrity warnings. They should be tracked, but only the full-edit deactivation bypass blocks this phase's must-have verification.

---

_Verified: 2026-04-25T12:51:25Z_  
