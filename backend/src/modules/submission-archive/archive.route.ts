...
export function serializeArchiveRow(row: ArchiveRouteRow) {
  return {
    ...
    personPhone: row.personPhone ?? null,
    status: row.status,
    tags: row.tags,
    processingSummary: row.processingSummary ?? null,
...
export function serializeArchiveDetail(detail: ArchiveRouteDetail) {
  return {
    ...
    processingFields: detail.processingFields,
    ...
    corrections: detail.correctionHistory.map((item) => ({
      id: item.id,
      changes: [
        {
          fieldId: item.field,
