<template>
  <q-page padding class="submissions-page">
    <div class="page-header row items-start q-mb-md q-gutter-sm">
      <div class="col min-width-0">
        <div class="text-h6">提交统计</div>
        <div class="text-body2 muted wrap-text">查看公开收集表单的提交记录和后续处理状态</div>
      </div>
      <q-btn
        v-if="isMobile"
        flat
      ...
      >
        <q-tooltip>筛选提交记录</q-tooltip>
      </q-btn>
      <q-btn flat dense round icon="refresh" aria-label="刷新提交记录列表" @click="refreshArchive">
        <q-tooltip>刷新提交记录列表</q-tooltip>
      </q-btn>
      <q-btn
    ...
    <div v-if="isDesktop" class="archive-filter row items-center q-gutter-sm q-mb-md">
      ...
      <q-select
        ...
        @update:model-value="applyFilters"
      />
      <q-btn color="primary" label="查询" class="archive-query" @click="queryArchive" />
    ...
    <q-card v-else-if="error" flat bordered class="state-panel text-center q-pa-xl">
      <div class="text-body1">提交记录加载失败，请检查网络后重试。</div>
      ...
      <div v-else class="q-gutter-sm">
        ...
        >
          <q-card-section>
            ...
            <div class="text-caption muted q-mt-sm wrap-text">提交人：{{ row.personName }}</div>
    ...
    <ArchiveStatsPanel
      v-perm="'form:submission:list'"
      aria-label="提交统计 按模板统计 按状态统计 按月份统计"
      :stats="store.stats"
    ...
    <q-dialog v-model="filterDialog" position="bottom">
      <q-card class="filter-sheet">
        ...
        <q-card-section class="text-h6">筛选提交记录</q-card-section>
        <q-card-section class="q-gutter-md">
          <q-select
            ...
            :options="store.filterOptions.templates"
          />
          <q-input v-model="filterDraft.personName" outlined dense clearable label="提交人" />
...
const columns = [
  ...
  { name: 'person', label: '提交人', field: 'personName', align: 'left' as const, style: 'width:180px' },
...
function readTemplateIdFromQuery(): number | null {
...
watch(
  ...
  (value) => {
    const next = readTemplateIdFromQuery();
    if (filters.templateId !== next) {
      filters.templateId = next;
      if (!firstLoading.value) queryArchive();
...
onMounted(() => {
  filters.templateId = readTemplateIdFromQuery();
  void loadArchive();
});
</script>
